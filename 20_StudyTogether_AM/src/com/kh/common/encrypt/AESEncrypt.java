...
//AES방식으로 양방향 암호화 하는 객체
	//생성과 동시에 암호화 키를 생성 또는 있는 키를 가져오기
	//함호화처리 메소드 작성
	//복호화처리(해독) 메소드 작성
	
	//키값을 저장하는 객체가 있음
	//SecretKey객체로 생성한 key값을 저장
	//*암호화 관련 패키지 javax.crypto패키지에 만들어져있음

	private static SecretKey key;
	private String path;
	
	public AESEncrypt(String path) { //listener에서
	...
	//key를 생성하는 매소드
		...
		public static void chageDecrypt(List<Member> list){
			for(Member m : list) {
				try {
